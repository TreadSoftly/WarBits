...
class TestUtilityPolicy(unittest.TestCase):
    def test_argmax_choice(self):
        ...
        def s1(ctx): return 0.2
        def s2(ctx): return 0.9
        def a1(ctx): ctx.bb.set("picked", "a1")
        def a2(ctx): ctx.bb.set("picked", "a2")

        policy = UtilityPolicy(actions=[
    ...
    def test_hysteresis_keeps_last(self):
        ...
        def s_hold(ctx): return 0.90
        def s_switch(ctx): return 0.92  # only slightly better
        def a_hold(ctx): ctx.bb.set("picked", "hold")
        def a_switch(ctx): ctx.bb.set("picked", "switch")
