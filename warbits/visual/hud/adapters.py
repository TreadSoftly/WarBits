...
def _extract_tracks(state: Any) -> Sequence[TargetTrack]:
    ...
    for key in ("enemy_ground", "ground", "ground_units"):
        ...
        for i, u in enumerate(_iterable_or_empty(units)):
            ...
            if pos is None:
                continue
            vel = _maybe_vec3(getattr(u, "vel", None)) or np.zeros(3, dtype=float)
    ...
    if enemies is not None:
        for i, e in enumerate(_iterable_or_empty(enemies)):
            if isinstance(e, dict):
                e_map = cast(Mapping[str, Any], e)
                pos = _maybe_vec3(e_map.get("pos"))
                vel = _maybe_vec3(e_map.get("vel")) or np.zeros(3, dtype=float)
            else:
                pos = _maybe_vec3(getattr(e, "pos", None))
                vel = _maybe_vec3(getattr(e, "vel", None)) or np.zeros(3, dtype=float)
            if pos is None:
                continue
