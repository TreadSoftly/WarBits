...
from warbits.visual.procedural.ordnance import MissileParams, BombParams, build_missile_blueprint, build_bomb_blueprint


def _assert_edges_in_range(V, E):
    n = int(V.shape[0])
    assert n > 0
    assert len(E) > 0
    for a, b in E:
...
def test_procedural_jet_blueprint_valid():
    ...
    assert bp.vertices_m.shape[1] == 3
