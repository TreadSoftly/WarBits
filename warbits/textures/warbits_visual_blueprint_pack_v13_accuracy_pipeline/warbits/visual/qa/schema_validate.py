...
def _as_vec3_list(v: Any) -> Optional[list[list[float]]]:
    if not isinstance(v, list):
        return None
    out: list[list[float]] = []
    rows = cast(list[object], v)
    if len(rows) == 0:
        return None
    for row in rows:
        if not isinstance(row, (list, tuple)):
            return None
        row_seq = list(cast(Iterable[Any], row))
        if len(row_seq) != 3:
            return None
...
def _as_edge_list(v: Any) -> Optional[list[list[int]]]:
    if not isinstance(v, list):
        return None
    out: list[list[int]] = []
    edges = cast(list[object], v)
    for e in edges:
        if not isinstance(e, (list, tuple)):
            return None
        pair = list(cast(Iterable[Any], e))
        if len(pair) != 2:
            return None
        a = pair[0]
        b = pair[1]
    ...
    with p.open("r", encoding="utf-8") as f:
        for idx, line in enumerate(f, start=1):
            ...
            if isinstance(rec, dict):
                yield idx, cast(Dict[str, Any], rec)
    ...
    try:
        for lineno, rec in _read_jsonl(blueprints_jsonl):
            if max_records is not None and res.total_records >= max_records:
                res.warnings.append(f"Stopped after max_records={max_records} (file may have more records).")
                break
            ...
            n_verts = len(verts)
            if n_verts < 2:
                res.errors.append(f"{blueprints_jsonl}:{lineno} ({blueprint_id}): too few vertices ({n_verts}).")
                continue
            ...
            if lods_raw is None:
                ...
                if not isinstance(lods_raw, dict):
                    ...
                    continue
                lods_map = cast(Dict[str, Any], lods_raw)
            ...
            if anchors is not None:
                ...
                for name, vec in anchors_map.items():
                    ...
                    if not isinstance(vec, (list, tuple)):
                        ...
                        continue
                    vec_seq = list(cast(Iterable[Any], vec))
                    if len(vec_seq) != 3:
                        res.errors.append(
                            f"{blueprints_jsonl}:{lineno} ({blueprint_id}): anchor '{name}' invalid vec3."
                        )
