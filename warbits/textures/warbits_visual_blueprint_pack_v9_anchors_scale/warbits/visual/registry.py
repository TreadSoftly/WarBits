...
class VisualRegistry:
    ...
    def get_cached(self, blueprint_id: str) -> CachedGeometry:
        ...
        if not isinstance(bp, Blueprint):
            # Should never happen if DB validates, but keep it defensive.
            raise TypeError(f"Blueprint record has unexpected type: {type(bp)}")
    ...
    def get_geometry(
        ...
        *,
        ...
        lod_key = self._lod_policy.select_lod(distance_m)
        E = cg.edges_by_lod.get(lod_key)
        if E is None:
            # If chosen LOD isn't present, fall back gracefully:
            E = cg.edges_by_lod.get("lod0") or cg.edges_by_lod.get("base")
            if E is None:
        ...
        return cg.vertices_m, E

    def get_bounds(self, blueprint_id: str) -> Tuple[np.ndarray, np.ndarray]:
        cg = self.get_cached(blueprint_id)
        return cg.bounds_min_m.copy(), cg.bounds_max_m.copy()

    def get_dims(self, blueprint_id: str) -> np.ndarray:
        cg = self.get_cached(blueprint_id)
        return cg.dims_m.copy()

    def get_center(self, blueprint_id: str) -> np.ndarray:
        cg = self.get_cached(blueprint_id)
