...
###############################################################################
# 3. CURRENT VEHICLE & LOADOUT (Hardcoded only for now!).:BEARCAT MASTER DATA
###############################################################################
F8F1_Bearcat_Data: Dict[str, Any] = {
    ...
    "Performance": {
        ...
        "L&ing_Distance_ft": 750,  # with typical approach flaps
...
###############################################################################
# 4. VEHICLE CONFIG (Pull from db or War Thunder Data Mine & plug into field)
###############################################################################
...
###############################################################################
# 7. PHASES (Map for scenario. A start to help with dynamics in scenarios)
###############################################################################
phases_info = {
    "Approach": ("blue",   60),
    "Strafe":   ("orange", 50),
...
###############################################################################
# 8. AIR ASSAULT - FLIGHT PATHS (Hardcoded for now. Need free flight & dynamic)
###############################################################################
def generate_path(
...
# # ###############################################################################
# # # 9 Marked Path Of Scenario (Turn on or off)
...
###############################################################################
# 10. GENERATE TERRAIN <NEEDS TO AUTO RECOGNIZE AND FILL FULL SCREEN>
###############################################################################
...
###############################################################################
# 11. PLOT 3D SCENES <Need this mode, PVP mode & Siumlator instruments mode>
###############################################################################
...
###############################################################################
# 12. GROUND ASSAULT <NEEDS DYNAMIC AIM, SHOOT/SHOT, HIT/MISS &/OR DESTROYED>
###############################################################################
...
###############################################################################
# 13. LOCKON & ENGAGEMENTS <ADD GROUND & LOCKON TARGET TO BLINK COLORS DYNAMIC>
###############################################################################
...
# Example request: "WHEN I FIRE THE BEARCAT BOMB OFF I NEED TO FIRE OFF A ROCKET
#  AT THE BOGIE OFF SCREEN AS I GO INTO THE ESCAPE PATH OFF OF THE BOMBING PATH..."
# You can handle that in #19 update_animation, after the bomb drop logic triggers.
...
def spawn_explosion(center: Tuple[float, float, float]):
    """
    Activates the explosion visualization at 'center' in 3D space,
    which the 'update_explosion()' function will animate.
    """
...
def spawn_parachute(center: Tuple[float, float, float]):
    """
    Example function to spawn a parachute from a bogie pilot or an ejected
    occupant. The 'update_parachute()' function will gently float it down.
    """
...
###############################################################################
# 15. ALL 3D MODELS (Current Hardcoded are Bearcat, Bogie. Add all others))
###############################################################################
def create_bearcat_model(
    ...
    This is a simplified shape (like a box with wings).
    """
    _, _, _ = position
    dx, dy, dz = direction
    ...
    poly.set_edgecolor((0, 0, 0, 1))  # black edge
    return poly

###############################################################################
# 16. BULLETS, BOMBS, ROCKETS, COUNTERMEASURES <Dynamic engagement ammo>
###############################################################################
...
###############################################################################
# 17. BOMB <Add the bullets and rockets and countermeasures here as well>
###############################################################################
...
bomb_x: np.ndarray = np.array([], dtype=np.float64)
bomb_y: np.ndarray = np.array([], dtype=np.float64)
bomb_z: np.ndarray = np.array([], dtype=np.float64)
...
def simulate_bomb_trajectory(
    ...
    """
    Compute a physically accurate projectile path for the bomb, given:
      - initial position (x_init, y_init, z_init)
      - initial velocity (vx_init, vy_init, vz_init)
      - gravity, drag
      - dt step, up to max_time
    """
...
def update_explosion():
    ...
    explosion_poly = ax_main.plot_surface(xs_, ys_, zs_, color='white',
...
###############################################################################
# 19. INIT + UPDATE (FINAL FIXED VERSION, with Bomb Logic Updated)
###############################################################################
...
def update_animation(frame: int) -> tuple[()]:
    ...
    if 0 <= bomb_frame_idx < len(bomb_x):
        ...
        # Implement the color cycling based on altitude
        # The closer to the ground, the faster it cycles between white, red, yellow, orange
        ...
        if alt > 0:
            bomb_marker_local = ax_main.scatter([bx_], [by_], [bz_], color=color, s=50, marker='o')
    ...
    if strafe_s <= frame < strafe_e:
        if lockon_line_radius > 0.0:
            ...
            lockon_line, = ax_main.plot(xx_, yy_, zz_, color='yellow', linewidth=1.5)
        ...
        if frame == strafe_e:
            ...
            target_destroyed_text = ax_main.text(
                10000, 7000, 100,
                "Target Destroyed",
                color='red',
                fontsize=16
    ...
    for bullet in active_bullets:
        if bullet['index'] < len(bullet['x']) - 1:
            ...
            if bullet['scatter'] is None:
                sc_ = ax_main.scatter([], [], [], color='yellow', marker='.', s=20)
                bullet['scatter'] = sc_
            bx_ = bullet['x'][idx_]
            by_ = bullet['y'][idx_]
            bz_ = bullet['z'][idx_]
            bullet['scatter']._offsets3d = (
                np.array([bx_]),
                np.array([by_]),
                np.array([bz_])
            )
    ...
    for rocket in active_rockets:
        if rocket['index'] < len(rocket['x']) - 1:
            ...
            if rocket['scatter'] is None:
                sc_ = ax_main.scatter([], [], [], color='magenta', marker='^', s=30)  # Initial color
                rocket['scatter'] = sc_
            ...
            rocket['scatter'].set_facecolor(color)  # Change the color of the rocket
